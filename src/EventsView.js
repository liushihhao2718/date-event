...
function EventsView() {
	...
	useEffect(()=>{
		if(!fetchEvents){
			...
			fetch('event.json').then(x=>x.json()).then(x=>{
				setEvents(x)
			})	
		}
	})
		...
		<>
			<main className="px-0 px-md-5 pt-3 pb-1 pb-md-5">
				<div className="calendar">
					...
					<MonthTitle />
					<DayBlock
						month={makeCalendarArray(date, events)}
						onClick={(d) => {
							handleShow(d);
						}}
					/>
				...
				<Modal.Body closeButton>
					<EventDiary
						events={events.filter(
							(ev) => ev.date === formatDate(showEvent)
						)}
					/>
				</Modal.Body>
			</Modal>
 ...
 */
function makeCalendarArray(date, events) {
...
function DayBlock(props) {
	...
	const days = month.map((x) => {
		return (
			...
			>
				<time
					dateTime={x.date.toLocaleDateString()}
					style={timeStyle(x)}
				>
					{x.text}
				</time>
